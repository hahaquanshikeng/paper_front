...
/***/ (function(module, exports, __webpack_require__) {
	...
	function Clustergrammer(args) {
	  ...
	  //搜索框垂直对齐
	  d3.select('.awesomplete').style('vertical-align', 'bottom').style('position', 'inherit');
	...
	module.exports = function make_matrix_rows(params, current_matrix, row_names = 'all', ds_level = -1) {
	  ...
	  if (make_tip) {
	    ...
	    var tip = d3_tip_custom().attr('class', function () {
	      ...
	      return class_string;
	    }).style('display', 'none').style('word-break', 'break-word').style('width', '500px').direction('nw').offset([0, 300]).html(function (d) {
	      var tooltip_string;
	      var col_data = null,
	          row_data = null;
	      ...
	      tooltip_string = `<p>行title:${row_data == null ? '暂无数据' : row_data.title}</p>
	    ...
	    if (d3.select(inst_selection).classed('hovering')) {
	      ...
	      if (inst_zoom === 0) {
	        ...
	        if (params.matrix.show_tile_tooltips && tip !== null) {
	          // matrix悬停背景框
	          d3.selectAll(params.viz.root_tips + '_tile_tip').style('display', 'block').style('background-color', 'rgba(194,213,238,0.5)').style('border-radius', '2px').style('opacity', '0.1').style('font-weight', 'bold');
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function make_col_label_container(cgm, text_delay = 0) {
	  ...
	  if (d3.select(params.root + ' .col_container').empty()) {
	    ...
	    .attr('width', 30 * params.viz.clust.dim.width + 'px').attr('height', params.viz.label_background.col).attr('class', 'white_bars').style('margin-top', '-50px');
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function (cgm, inst_order, inst_rc) {
	  ...
	  if (other_rc === 'row') {
	    ...
	    //联动
	    var order_list = params.matrix.orders[params.viz.inst_order.row + '_row'];
	    var col_name_list = [];
	    for (var order of order_list) {
	      col_name_list.push(params.inst_nodes.col_nodes[order].name);
	    }
	    matrixFront.col_reorder_callback(col_name_list);
	  } else if (other_rc == 'col') {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function resize_viz(cgm) {
	  ...
	  .style('margin-top', '-50px').style('width', cont_dim.width + 'px').style('height', cont_dim.height + 'px');
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function make_col_cat(cgm) {
	  ...
	  var cat_tip = d3_tip_custom().attr('class', function () {
	    ...
	    return class_string;
	  }).style('background-color', 'rgba(194,213,238,0.5)').style('font-weight', 'bold').direction('s').offset([5, 0]).style('display', 'none').html(function (d) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function ini_sidebar(cgm) {
	  ...
	  // 搜索内容自动补全（使用title）
	  // 行标题搜索
	  var entities = cgm.params.network_data.row_nodes.map(t => t["cat-0"].split("title: ")[1]);
	...
	module.exports = function run_row_search(cgm, search_term, entities) {
	  if (entities.indexOf(search_term) !== -1) {
	    ...
	    d3.selectAll(cgm.params.root + ' .row_label_group').filter(function (d) {
	      // 行标题搜索
	      return d["cat-0"].split("title: ")[1] === search_term;
...
/***/ (function(module, exports, __webpack_require__) {
	...
	if(false) {
		...
		if(!content.locals) {
			module.hot.accept("!!../../node_modules/_css-loader@0.23.1@css-loader/index.js!./d3.slider.css", function() {
				var newContent = require("!!../../node_modules/_css-loader@0.23.1@css-loader/index.js!./d3.slider.css");
...
/***/ (function(module, exports, __webpack_require__) {
	...
	if(false) {
		// When the styles change, update the <style> tags
		if(!content.locals) {
			module.hot.accept("!!../_css-loader@0.23.1@css-loader/index.js!./awesomplete.css", function() {
				var newContent = require("!!../_css-loader@0.23.1@css-loader/index.js!./awesomplete.css");
				if(typeof newContent === 'string') newContent = [[module.id, newContent, '']];
				update(newContent);
			});
		}
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function sidebar(cgm) {
	  ...
	  make_colorbar(cgm);
	  //顶部边栏收展
	  sidebar.append("div").classed('angle-double-container', true).classed('angle-double-container-down', true).style("text-align", "center").append('div').classed('angle-double-button', true).classed('list-down', true).html('<i class="fa fa-angle-double-down" aria-hidden="true"></i>').style('display', 'inline-block');
...
/***/ (function(module, exports) {
	...
	module.exports = function set_up_search(sidebar, params) {
	  var search_container = sidebar.append('div').classed('gene_search_container', true).style('padding-left', '5%').style('padding-right', '30px').style('margin-top', '10px');
	  // .style('margin-bottom','10px')

	  search_container.append('input').classed('form-control', true).classed('gene_search_box', true).classed('sidebar_text', true).attr('type', 'text').attr('placeholder', "行搜索").style('height', params.sidebar.row_search.box.height + 'px').style('float', 'left').style('margin-top', '10px');

	  search_container.append('div').classed('gene_search_button', true).style('display', 'inline-block').style('margin-left', '5px').style('vertical-align', 'bottom')
	  // .style('float', 'left')
	  .attr('data-toggle', 'buttons').append('button').classed('sidebar_text', true).html('搜索').attr('type', 'button').classed('btn', true).classed('btn-primary', true).classed('submit_gene_button', true).style('width', '100%').style('font-size', '14px');
		};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function set_up_reorder(params, sidebar) {
	  ...
	  var inst_reorder;
	  // 自定义附加功能容器
	  var func_container = sidebar.append('div').style('width', '100%').style('padding-right', '30px').style('height', '34px').style('margin-bottom', '10px');
	  // 原始矩阵
	  func_container.append('button').attr('type', 'button').style('float', 'left').style('width', '20%').style('margin-left', '5%').classed('btn', true).classed('btn-primary', true).classed('sidebar_button_text', true).html("原始矩阵");
	  //参考扩展
	  func_container.append('button').attr('type', 'button').style('float', 'left').style('width', '20%').style('margin-left', '5%').classed('btn', true).classed('btn-primary', true).classed('sidebar_button_text', true).html("参考扩展");
	  //引用补充
	  func_container.append('button').attr('type', 'button').style('float', 'left').style('width', '20%').style('margin-left', '5%').classed('btn', true).classed('btn-primary', true).classed('sidebar_button_text', true).html("引用补充");
	  //自动迭代
	  func_container.append('button').attr('type', 'button').style('float', 'left').style('width', '20%').style('margin-left', '5%').classed('btn', true).classed('btn-primary', true).classed('sidebar_button_text', true).html("自动迭代");

	  var reorder_section = sidebar.append('div').style('padding-right', '30px').classed('reorder_section', true);
	  // 求和与聚类是否分行列
	  // params.sim_mat=true
	  var reorder_types;
	  if (params.sim_mat) {
	  ...
	  underscore.each(reorder_types, function (inst_rc) {
	    ...
	    inst_reorder = reorder_section.append('div').classed('btn-group-vertical', true).style('width', '50%').classed('toggle_' + inst_rc + '_order', true).attr('role', 'group');

	    inst_reorder.selectAll('.button').data(possible_orders).enter().append('button').attr('type', 'button').style('float', 'left').style('width', '40%').style('margin-left', '10%').classed('btn', true).classed('btn-primary', true).classed('sidebar_button_text', true).classed('active', function (d) {
	      is_active = false;
